...
module Row = struct
  ...
  let parse_airports (country_map : string String.Map.t) : t Delimited_kernel.Read.t =
    ...
    { code ; name ; city ; country ; keywords}
  ;;

  let parse_citycodes (country_map : string String.Map.t) : t Delimited_kernel.Read.t =
    let open Delimited_kernel.Read.Let_syntax in
    ...
    if String.is_empty code then failwith "No code" else
    { code ; name ; city ; country ; keywords = None}
  ;;
...
let get_all_airports () =
  ...
  let%bind airports_list = read_csv ~filename:"goated_airports.csv" ~country_map ~parse:Row.parse_airports in 
  let%bind citycodes_list = read_csv ~filename:"citycodes.csv" ~country_map ~parse:Row.parse_citycodes in
